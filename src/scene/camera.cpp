...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py) {
	...
	float h = tan(Radians(vertical_fov / 2));
	float w = h * aspect_ratio;
	Vec3 sensor_point(
		- w / 2.0f + sensor_pixel.x / film.width * w,
		- h / 2.0f + sensor_pixel.y / film.height * h,
		- 1.0f
	);
	...
	ray.dir = sensor_point.unit(); //TODO: compute from sensor plane position
